...
class SampleForm:
    def __init__(self, root, sample_controller, on_home):
        ...
        tk.Label(frame, text="SAMPLE FORM", font=("Arial", 40),
        ...
        tk.Button(frame, text="Add Sample",width=20,font=("Arial",10),
                  command=self.add_sample).grid(row=2, column=0, columnspan=2,pady=10)

        # ── SEARCH (معكوس) ──
        tk.Button(frame, text="Search", width=20, font=("Arial",10),
                  command=self.search).grid(row=3, column=0,pady=5)

        self.search_entry = tk.Entry(frame, font=("Arial",12))
        self.search_entry.grid(row=3, column=1, pady=5)

        # ── FILTER (معكوس) ──
        tk.Button(frame, text="Filter", width=20, font=("Arial",10),
                  command=self.filter).grid(row=4, column=0,pady=5)
        ...
        # ── BUTTONS ──
        tk.Button(frame, text="Refresh",width=20,font=("Arial",10),
                  command=self.load_samples).grid(row=6, column=0,pady=10)

        tk.Button(frame, text="Home",width=20,font=("Arial",10),
                  command=self.on_home).grid(row=6, column=1,pady=10)

        tk.Button(frame, text="Processing",width=20,font=("Arial",10),
                  command=self.mark_processing).grid(row=7, column=0,pady=5)

        tk.Button(frame, text="Completed",width=20,font=("Arial",10),
                  command=self.mark_completed).grid(row=7, column=1,pady=5)
